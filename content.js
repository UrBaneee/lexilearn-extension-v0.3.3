(async () => {
  ...
  root.setAttribute(LEXI_SENTINEL, 'alive');
  console.log('[Lexi] initializing fresh content script...');

  // === 模式初始化 ===
  let HIGHLIGHT_MODE = 'basic';
  (async () => {
    try {
      const { mode } = await chrome.storage.local.get({ mode: 'basic' });
      HIGHLIGHT_MODE = mode;
      console.log('[Lexi] highlight mode:', HIGHLIGHT_MODE);
    } catch (e) {
      console.warn('[Lexi] failed to load highlight mode:', e);
    }
  })();
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area === 'local' && changes.mode) {
      HIGHLIGHT_MODE = changes.mode.newValue || 'basic';
      console.log('[Lexi] highlight mode changed to:', HIGHLIGHT_MODE);
    }
  });
  ...
  async function safeLoadList(relPath) {
    try {
      const url = chrome.runtime.getURL(relPath);
      const res = await fetch(url);
      if (!res.ok) return new Set();                     // 404/403 → 空集合
      const txt = await res.text();
      return new Set(
        txt.split(/\r?\n/).map(s => s.trim()).filter(Boolean)
      );
    } catch (e) {
      console.warn('[Lexi] list load failed:', relPath, e);
      return new Set();
    }
  }

  // 同步/初始化时调用
  const [CET4, EXTRA, TOP5K, GRE] = await Promise.all([
    safeLoadList('assets/lists/cet4.txt'),          // 可没有
    safeLoadList('assets/lists/stopwords_extra.txt'),  // 可没有
    safeLoadList('assets/lists/freq_top5000.txt'),  // 可没有
    safeLoadList('assets/lists/gre.txt'),           // GRE 模式需要这个
  ]);
      ...
      chrome.runtime.sendMessage(
        ...
        `;
          ...
          addMenu.addEventListener("click", async (ev) => {
            ...
            // 5) 先保存，再刷新 sidepanel、再打开 sidepanel
            await chrome.runtime.sendMessage({ type: 'ADD_VOCAB', payload });
            await chrome.runtime.sendMessage({ type: 'VOCAB_UPDATED' });
            await chrome.runtime.sendMessage({ type: 'OPEN_SIDEPANEL' })
              .then(r => console.log('[Lexi] OPEN_SIDEPANEL response:', r))
              .catch(e => console.warn('[Lexi] OPEN_SIDEPANEL error:', e));
            });
